    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-b from-[#F4F8FC] via-[#EEF4FB] to-[#E8F0FA] px-6 py-10">
      ...
      <main className="relative w-full max-w-md rounded-3xl border border-white/60 bg-white/90 p-8 shadow-[0_20px_70px_rgba(41,74,110,0.15)] backdrop-blur-sm sm:p-10">
        <header className="mb-8 text-center">
          <p className="text-sm font-medium tracking-[0.2em] text-[#5E7FA2]">CONNEXION</p>
          <h1 className="mt-3 text-4xl font-semibold tracking-tight text-[#17324D]">2nest 🪺</h1>
          <p className="mt-3 text-sm text-[#5D738A]">Heureux de vous revoir.</p>
        </header>
        ...
        <form className="space-y-5" onSubmit={onSubmit}>
          ...
          {errorMessage && (
            <p className="rounded-xl border border-[#E3B4B8] bg-[#FFF4F5] px-3 py-2 text-sm text-[#8D3E45]">{errorMessage}</p>
          )}

          <button
            type="submit"
            disabled={isLoading || Boolean(configError)}
            className="mt-2 w-full rounded-xl bg-[#4A90D9] px-4 py-3 text-sm font-semibold text-white shadow-[0_10px_24px_rgba(74,144,217,0.35)] transition hover:brightness-105 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#4A90D9]/30 disabled:cursor-not-allowed disabled:opacity-70"
          >
        ...
        <p className="mt-6 text-center text-sm text-[#5D738A]">
          ...
          <Link href="/signup" className="font-semibold text-[#2F6FAF] underline-offset-4 transition hover:underline">
            S'inscrire
          </Link>
        </p>
      </main>
